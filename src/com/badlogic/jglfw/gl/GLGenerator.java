...
import com.badlogic.jglfw.gl.GLParser.GLConstant;
import com.badlogic.jglfw.gl.GLParser.GLParam;
import com.badlogic.jglfw.gl.GLParser.GLProcedure;
...
public class GLGenerator {
	...
	private Map<String, String> readCustomProcs () {
		...
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(GLGenerator.class.getResourceAsStream("/com/badlogic/jglfw/gl/custom.txt")));
			while(true) {
	...
	private void generatePreamble (StringBuffer buffer) {
		try {
			buffer.append("\t// @off\n");
			buffer.append("\t/*JNI\n"); 
			buffer.append("\t#include <GL/glfw.h>\n");
